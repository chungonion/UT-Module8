...
public class GenericLLParser implements Parser {
    ...
    private AST parse(Symbol symb) throws ParseException {
        ...
        if (symb instanceof NonTerm) {
            ...
            //Get the non term of the previous one
        }
    ...
    private Map<NonTerm, Map<Term, Rule>> calcLL1Table() {
        ...
        for (NonTerm nonTerm : g.getNonterminals()) {
            Map<Term, Rule> newRules = new HashMap<>();
            for (Rule rule : g.getRules(nonTerm)) {
                for (Term term : calc.getFirstp().get(rule)) {
                    newRules.put(term, rule);
                }

                if (calc.getFirstp().get(rule).contains(Symbol.EOF)) {
                    newRules.put(Symbol.EOF, rule);
                }
            }
            ll1Table.put(nonTerm, newRules);

        }
        return ll1Table; // TODO fill in
    }

    //Testing of CC2, see whether identical to CC2Parser
