...
import static pp.block2.cc.ll.CC2.*;
...
public class CC2Parser implements Parser {
	...
	private static final NonTerm NT_L = new NonTerm("L");
	private static final NonTerm NT_C = new NonTerm("C");
	private static final NonTerm NT_R = new NonTerm("R");
	private static final NonTerm NT_R_PRIME = new NonTerm("R'");
	private static final NonTerm NT_Q = new NonTerm("Q");
	...
	private AST parseL() throws ParseException{
		...
		switch (next.getType()) {
			...
			case B:
				...
				break;
			default:
				throw unparsable(NT_L);
		}
		return result;
	}

	private AST parseC() throws ParseException{
		AST result = new AST(NT_C);
		Token next = peek();
		switch (next.getType()) {
			case C:
